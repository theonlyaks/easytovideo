...
export function Plans({ user }: AuthState) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:mt-0">
      ...
      <div className="flex justify-center w-full">
        <div className="max-w-md">
          <PlanCard key={plans[0].id} {...plans[0]} />
        </div>
      </div>
      <div className="text-center space-y-2 mt-6 sm:mt-8 px-4 sm:px-0">
        <p className="text-sm ">
          Note: USD prices shown are approximate. Actual charges may vary based
          on current exchange rates at the time of payment.
        </p>
      </div>
    </div>
  );
