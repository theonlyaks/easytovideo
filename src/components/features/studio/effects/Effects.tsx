...
export function Effects({user}: AuthState) {
  ...
  const isInvalidDuration = duration > 0 && isClipTooLong(timeRange.start, timeRange.end, MAX_DURATION_SECONDS);
  ...
  return (
    <main className="hidden md:block max-w-4xl mx-auto py-12">
      <h2 className="text-2xl font-semibold mb-8">Video Effects</h2>

      {!videoUrl ? (
        <button
          onClick={() => setIsFileManagerOpen(true)}
          className="w-full border-2 border-dashed border-primary rounded-lg p-8 text-center cursor-pointer hover:bg-primary/5 transition-colors"
        >
          <MdAdd className="mx-auto h-12 w-12 text-primary mb-2" />
          <p className="text-lg mb-2">Select video from library</p>
          <p className="text-sm text-muted-text">Supports MP4, WebM, and Ogg</p>
        </button>
      ) : (
        <div className="space-y-4">
          ...
          {duration > 0 && (
            <div className="w-full space-y-4"> {/* Changed from max-w-[853px] to w-full */}
              <RangeSeeker
                min={0}
                max={duration}
